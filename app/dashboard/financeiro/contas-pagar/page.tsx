...
export default function ContasPagarPage() {
  ...
  const [filtros, setFiltros] = useState({
    ...
    categoria: "",
    busca: "",
    tipo_busca: "descricao", // descricao, proposta, beneficiario, dependente
  })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingConta ? `/api/financeiro/contas-pagar/${editingConta.id}` : "/api/financeiro/contas-pagar"
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch(`/api/financeiro/contas-pagar/${deletingId}`, {
        method: "DELETE",
      })
  ...
  const handleExport = () => {
    ...
    a.href = url
    a.download = `contas-pagar-${format(new Date(), "yyyy-MM-dd")}.csv`
    a.click()
  ...
  const openEditDialog = (conta: ContaPagar) => {
  ...
  const getStatusBadge = (status: string) => {
    const badges = {
      ...
      programado: <Badge className="bg-purple-100 text-purple-800 hover:bg-purple-100">Programado</Badge>,
    }
  ...
  const totalPago = contas.filter((c) => c.status === "pago").reduce((sum, c) => sum + c.valor_pago, 0)
    ...
    <div className="flex-1 space-y-6 p-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Gerencie suas contas a pagar e controle despesas</p>
        </div>
        <Button
      ...
      <Tabs defaultValue="lista" className="space-y-4">
        ...
        <TabsContent value="filtros" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Configure filtros personalizados incluindo busca por proposta, beneficiário ou dependente
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid gap-4 md:grid-cols-4">
                <div className="space-y-2">
                  <Label htmlFor="tipo_busca">Tipo de Busca</Label>
                  <Select
                    value={filtros.tipo_busca}
                    onValueChange={(value) => setFiltros({ ...filtros, tipo_busca: value })}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                ...
                <div className="space-y-2">
                  <Label htmlFor="busca">
                    {filtros.tipo_busca === "proposta" && "Número da Proposta"}
                    {filtros.tipo_busca === "beneficiario" && "Nome do Beneficiário"}
                    {filtros.tipo_busca === "dependente" && "Nome do Dependente"}
                    {filtros.tipo_busca === "descricao" && "Descrição"}
                  </Label>
                  <div className="relative">
                    <Search className="absolute left-3 top-3 h-4 w-4 text-muted-foreground" />
                    <Input
                      id="busca"
                      placeholder={`Buscar por ${filtros.tipo_busca}...`}
                      value={filtros.busca}
              ...
              <div className="flex gap-4">
                ...
                <Button
                  variant="outline"
                  onClick={() => {
                    setFiltros({ status: "", categoria: "", busca: "", tipo_busca: "descricao" })
                  }}
                >
                  Limpar Filtros
                </Button>
        ...
        <TabsContent value="relatorios" className="space-y-4">
          <Card>
            <CardHeader>
              <CardTitle>Relatórios de Contas a Pagar</CardTitle>
              <CardDescription>Gere relatórios detalhados sobre suas contas a pagar</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid gap-4 md:grid-cols-2">
                <Card>
                  <CardHeader>
                    <CardTitle className="text-lg">Relatório de Despesas</CardTitle>
                    <CardDescription>Análise detalhada das despesas por categoria</CardDescription>
                  </CardHeader>
                  <CardContent>
                ...
                <Card>
                  <CardHeader>
                    <CardTitle className="text-lg">Fluxo de Pagamentos</CardTitle>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Preencha os dados da conta a pagar (restituições, reembolsos, fornecedores, etc.)
            </DialogDescription>
          </DialogHeader>
        ...
        <DialogContent className="max-w-2xl">
          ...
          {viewingConta && (
            <div className="grid gap-4 py-4">
              ...
              {viewingConta.beneficiario_nome && (
                <div>
                  <Label className="text-muted-foreground">Beneficiário</Label>
                  <p className="font-medium">{viewingConta.beneficiario_nome}</p>
                </div>
              )}
              {viewingConta.proposta_numero && (
                <div>
                  <Label className="text-muted-foreground">Proposta</Label>
                  <p className="font-medium">{viewingConta.proposta_numero}</p>
                </div>
              )}
