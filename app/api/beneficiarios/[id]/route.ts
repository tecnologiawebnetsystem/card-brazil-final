...
export async function DELETE(_: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { id } = await params
    const rows = await query(
      "UPDATE beneficiarios SET deleted_at = CURRENT_TIMESTAMP, updated_at = CURRENT_TIMESTAMP, status = 'inativo' WHERE id = $1 AND administradora_id = $2 AND deleted_at IS NULL RETURNING id",
      [Number.parseInt(id, 10), administradoraId],
    )
    return rows[0]
