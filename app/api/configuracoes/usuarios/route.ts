...
export async function POST(request: NextRequest) {
  try {
    ...
    await recordCadastroAudit({ administradoraId, userId, action: "create", tableName: "usuarios", recordId: rows[0].id, after: rows[0] })
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const current = await query(`SELECT ${publicFields} FROM usuarios WHERE id = $1 AND administradora_id = $2 AND deleted_at IS NULL`, [body.id, administradoraId])
    if (!current[0]) return NextResponse.json({ error: "Usuário não encontrado." }, { status: 404 })
    ...
    values.push(administradoraId)
    const idPosition = values.length - 1
    const rows = await query(`UPDATE usuarios SET nome_completo=$1, email=$2, tipo_usuario=$3, status=$4${passwordSql}, updated_at=CURRENT_TIMESTAMP WHERE id=$${idPosition} AND administradora_id=$${idPosition + 1} AND deleted_at IS NULL RETURNING ${publicFields}`, values)
    if (rows[0]) await recordCadastroAudit({ administradoraId, userId, action: "update", tableName: "usuarios", recordId: rows[0].id, before: current[0], after: rows[0] })
...
export async function DELETE(request: NextRequest) {
  try { const { administradoraId, userId } = await requireCadastroAccess("delete"); const id = Number(request.nextUrl.searchParams.get("id")); if (!id) return NextResponse.json({ error: "ID inválido." }, { status: 400 }); const current = await query(`SELECT ${publicFields} FROM usuarios WHERE id = $1 AND administradora_id = $2 AND deleted_at IS NULL`, [id, administradoraId]); const rows = await query("UPDATE usuarios SET deleted_at=CURRENT_TIMESTAMP, status='inativo', updated_at=CURRENT_TIMESTAMP WHERE id=$1 AND administradora_id=$2 AND deleted_at IS NULL RETURNING id", [id, administradoraId]); if (!rows.length) return NextResponse.json({ error: "Usuário não encontrado." }, { status: 404 }); await recordCadastroAudit({ administradoraId, userId, action: "delete", tableName: "usuarios", recordId: id, before: current[0], after: { status: "inativo", deleted_at: "CURRENT_TIMESTAMP" } }); return NextResponse.json({ success: true }) } catch (error: any) { const auth = apiAuthError(error); return NextResponse.json({ error: auth?.message || "Não foi possível excluir o usuário." }, { status: auth?.status || 500 }) }
}
