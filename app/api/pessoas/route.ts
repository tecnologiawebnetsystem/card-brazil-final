...
export async function GET(request: NextRequest) {
  try {
    ...
    const resultado = await query(`SELECT p.*, COALESCE(p.nome_completo, p.razao_social, p.nome_fantasia, '') AS nome_exibicao FROM pessoas p${where ? where.replace(/\b(nome_completo|razao_social|cpf|cnpj|email)\b/g, 'p.$1').replace(/nome_completo/g, 'nome') : ''} ORDER BY p.created_at DESC NULLS LAST`, params)
