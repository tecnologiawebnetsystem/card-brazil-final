...
export async function GET() {
  try {
    ...
    const [beneficiarios, propostas, recebiveis, pagamentos, contratos, usuarios] = await Promise.all([
      ...
      query<{ aberto: string; vencido: string; recebido: string }>(`SELECT COALESCE(SUM(valor_total) FILTER (WHERE status IN ('pendente','vencida')),0)::numeric AS aberto, COALESCE(SUM(valor_total) FILTER (WHERE status = 'vencida'),0)::numeric AS vencido, COALESCE(SUM(valor_pago),0)::numeric AS recebido FROM contas_receber WHERE id_administradora = $1`, [administradoraId]),
      query<{ total: string; pago: string }>(`SELECT COUNT(*)::int AS total, COUNT(*) FILTER (WHERE status IN ('pago','liquidado'))::int AS pago FROM contas_pagar WHERE id_administradora = $1`, [administradoraId]),
