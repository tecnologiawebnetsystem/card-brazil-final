...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (body.valor_original || body.valor_multa || body.valor_juros || body.valor_desconto) {
      const [current] = await pool.execute(`SELECT * FROM contas_receber WHERE id = ?`, [id])
    ...
    const [result] = await pool.execute(`UPDATE contas_receber SET ${setClause} WHERE id = ?`, [...values, id])
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    // Soft delete
    const [result] = await pool.execute(`UPDATE contas_receber SET deleted_at = NOW() WHERE id = ?`, [id])
