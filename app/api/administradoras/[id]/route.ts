...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try { const id = Number.parseInt((await params).id, 10); const updated = await service.update(id, await request.json()); return updated ? NextResponse.json(successResponse(updated, "Administradora atualizada com sucesso")) : NextResponse.json(errorResponse("Administradora não encontrada"), { status: 404 }) } catch (error) { return NextResponse.json(handleApiError(error), { status: 500 }) }
}
