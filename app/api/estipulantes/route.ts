...
export async function GET(request: NextRequest) {
  try {
    ...
    const estipulantes = await estipulantesService.findAll(filters)
...
export async function POST(request: NextRequest) {
  try {
    ...
    const id = await estipulantesService.create({
      ...body,
      created_at: new Date(),
      updated_at: new Date(),
    })
    ...
    return NextResponse.json(successResponse(estipulante, "Estipulante criado com sucesso"), { status: 201 })
  } catch (error) {
    return NextResponse.json(handleApiError(error), { status: 500 })
  }
}
