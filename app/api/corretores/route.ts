...
import { mockCorretores } from "@/lib/mock-data"
...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = mockCorretores.map((item) => ({ ...item, pessoa_id: item.id, status: item.ativo ? "ativo" : "inativo", nome_exibicao: item.nome, registro_susep: item.susep }))
